...
input = _.orderBy input

updateBots = (instruction) ->
  botNumbers = instruction.split ' '
  firstBot = parseInt botNumbers[1]

  isOutputLow = botNumbers[5] is 'output'
  lowBot = parseInt botNumbers[6]

  isOutputHigh = botNumbers[10] is 'output'
  highBot = parseInt botNumbers[11]
...
setValue = (instruction) ->
  ...
  bot = parseInt instruction.split(' ')[5]
  bots[bot] ?= {}
  bots[bot].values ?= []
  bots[bot].values.push value
  bots[bot].values = _.sortBy bots[bot].values
...
_.each bots, (bot, index) ->
  console.log "bot #{bot.index}, #{bot?.low?.index}, #{bot?.high?.index}, #{bot?.values}"
