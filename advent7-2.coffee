...
getPossibleABA = (s) ->
  abas = []
  abasRegex = /([a-z])([a-z])\1/g
  while (aba = abasRegex.exec(s))?
    if aba[1] isnt aba[2]
      console.log 'found aba', aba[0]
      abasRegex.lastIndex -= 2
      abas.push aba[0]
  abas
...
testString = (s) ->
  inners = []
  innerRegex = /\[([a-z]*)\]/g
  while (inner = innerRegex.exec(s))?
    console.log 'found inner', inner[1]
    inners.push inner[1]

  outers = []
  outerRegex = /([a-z]*)\[/g
  while (outer = outerRegex.exec(s))?
    console.log 'found outer', outer[1]
    outers.push outer[1]

  last = /\]([a-z]*)$/.exec(s)?[1]
  if last?
    console.log 'found outer', last
    outers.push last

  outersAbas = _.flatten _.map outers, getPossibleABA
  innersAbas = _.flatten _.map inners, getPossibleABA
  console.log outersAbas
  console.log innersAbas
  _.intersection(outersAbas, _.map(innersAbas, reverseAba)).length > 0
...
console.log checks
console.log _.compact(checks).length
